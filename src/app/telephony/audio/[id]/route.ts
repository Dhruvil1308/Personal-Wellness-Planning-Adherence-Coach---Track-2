 ...
 * fetches it directly over the tunnel — so it exposes nothing but the audio for
 * a known cuid, and only while a call is in flight (the hangup callback deletes
 * the file).
 */
export async function GET(_req: Request, ctx: RouteContext<"/telephony/audio/[id]">) {
...
export async function HEAD(_req: Request, ctx: RouteContext<"/telephony/audio/[id]">) {
  const { id } = await ctx.params;
  const wav = await readReminderAudio(id);
